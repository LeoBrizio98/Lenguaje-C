...
bool lookup(string s)
{
    ...
    while (min <= max)
    {
        ...
        dictionary.size = mid; // El indice numerico del diccionario
        char *wrd = (dictionary.words[dictionary.size]).letters; // Busca la palabra en el diccionario y la guarda
        comp = strcmp(s, wrd);
...
// Funcion midpoint para el algoritmo de busqueda binaria
int midpoint(int x, int y)
{
    return ((x + y) / 2);
}