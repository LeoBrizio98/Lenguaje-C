...
struct
{
    // The current level // El nivel actual
    char *level;

    // The game's board // El tablero de juego
    int board[9][9];

    // The board's number // Los numeros de tablero
    int number;

    // The board's top-left coordinates // Las coordenadas de la parte superior izquierda
    int top, left;

    // The cursor's current location between (0,0) and (8,8) // La ubicacion actual del cursor entre (0,0) y (8,8)
    int y, x;
} g;
...
int main(int argc, char *argv[])
{
    ...
    // Ensure that level is valid // Verifica si el nivel es valido
    ...
    // n00b and l33t levels have 1024 boards; debug level has 9 // Si el comando es debug establece el limite de niveles en 9, si no el limite es 1024
    int max = (strcmp(g.level, "debug") == 0) ? 9 : 1024;

    // Ensure that #, if provided, is in [1, max] // Verifica que el segundo argumento este entre 1 y el maximo, ya sea 9 o 1024
    if (argc == 3)
    {
        // Ensure n is integral // Verifica que n sea entero
        char c;
        if (sscanf(argv[2], " %d %c", &g.number, &c) != 1) // sscanf Devuelve la cantidad de elementos, si es diferente de 1 entonces falla
        {
            fprintf(stderr, "Usage: sudoku n00b|l33t [#]\n");
            return 3;
        }

        // Ensure n is in [1, max] // Verifica que n este entre [1,max]
        if (g.number < 1 || g.number > max)
        {
            fprintf(stderr, "That board # does not exist!\n"); // Imprime que el tablero no existe si n es mayor a max
            return 4;
        }

        // Seed PRNG with # so that we get same sequence of boards // Se utiliza para obtener una secuencia de tableros
        srand(g.number);
    }
    else
    {
        // Seed PRNG with current time so that we get any sequence of boards // Utiliza la hora para obtener un numero aleatorio de tablero
        srand(time(NULL));

        // Choose a random n in [1, max] // Elige un numero random entre 1 y max
        g.number = rand() % max + 1;
    }

    // Start up ncurses // Inicia el ncurses
    if (!startup())
    {
        fprintf(stderr, "Error starting up ncurses!\n"); // Si por alguna razon el startup falla indica error
        return 5;
    }

    // Register handler for SIGWINCH (SIGnal WINdow CHanged) // Cambio de tamaño en terminal y ajustar diseño de interfaz
...
void checkrows(int val);
{
    ...
    for(int col = g.x; col < aux; col = col + 10)
    {
        for(int fila = 0; fila < aux; fila++)
        {
            if (g.board[fila][col] == val)
            {
                compare = false;
                break;
            }
